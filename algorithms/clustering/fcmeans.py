...
class FCMeans(object):
    ...
    def init_centroids(self):
        ...
        for idx_ in self.centroids:
            for idx, xi in enumerate(self.data):
                ...
                all_norms = [
                    norm / np.linalg.norm(xi - self.centroids[c]) for c in self.centroids]
    ...
    def fit(self, data=None, k=None):
        ...
        for iteration in range(self.n_iter):
            ...
            self.all_centroids[iteration] = dict(self.centroids)
            self.get_clusters()
            self.all_clusters[iteration] = self.clusters
            ...
            for idx_ in self.centroids:
                for idx, xi in enumerate(self.data):
                    ...
                    all_norms = np.power(all_norms, 2 / (self.fuzzy_c - 1))
                    updated_degree_of_membership = 1 / sum(all_norms)
                    diff = updated_degree_of_membership - \
                        self.degree_of_membership[idx][idx_]
                    self.degree_of_membership[idx][idx_] = updated_degree_of_membership
